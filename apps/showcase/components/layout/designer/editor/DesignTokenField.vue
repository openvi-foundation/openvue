<template>
    <div class="group">
        <div class="flex justify-between justify-items-center">
            ...
            <button v-if="switchable" type="button" @click="transfer" tabindex="-1" class="shrink-0 w-4 h-4 inline-flex items-center justify-center">
                <i class="pi pi-sort-alt !text-xs text-zinc-500 dark:text-white/50 opacity-0 group-hover:opacity-100 transition-opacity duration-150" title="Transfer between color scheme and common"></i>
            </button>
        </div>
...
export default {
    ...
    created() {
        ...
        // Not reactive: it is a callback held between focus and blur, never rendered.
    ...
    methods: {
        ...
        /**
         * Capture the pre-edit value when the field gains focus, so the whole focus session
         * collapses into a single undo step rather than one per keystroke.
         */
        onFocus() {
            const path = this.historyPath || this.path;

            if (path) {
                this.commitEdit = this.designerService.recordEdit(path);
            }
        },
        onBlur() {
            if (this.commitEdit) {
                this.commitEdit(this.modelValue);
                this.commitEdit = null;
            }
