    ...
    <section>
        ...
        <div v-if="hasPrimitiveTokens" class="grid grid-cols-4 gap-x-2 gap-y-3">
            <template v-for="(t_value, t_name) in primitiveTokens" :key="t_name">
                <DesignTokenField v-model="tokens[t_name]" :name="t_name" :label="camelCaseToSpaces(t_name)" :path="path + '.' + t_name" :historyPath="'semantic.' + path + '.' + t_name" :switchable="switchable" />
            </template>
        </div>
