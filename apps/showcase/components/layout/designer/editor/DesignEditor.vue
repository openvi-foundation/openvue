...
export default {
    ...
    computed: {
        /**
         * Match the doc-page convention used by DocComponent.getComponentName(): the lowercased
         * last path segment. That is the same key space as the preset's `components` map, so no
         * mapping table is needed. Using $route.name instead would diverge on nested routes
         * (`/theming/styled` yields the name `theming-styled`).
         */
        componentName() {
        ...
        isComponentRoute() {
            return this.componentName != null && this.$appState.designer.theme.preset?.components?.[this.componentName] != null;
        }
    }
};
</script>
